...
export class DashboardRepository {
  ...
  async getAllPaymentsWithTransactions(startDate?: Date, endDate?: Date) {
    const whereCondition: any = {}

    if (startDate || endDate) {
      whereCondition.payment_transaction = {
        transactionDate: {},
      }

      if (startDate) {
        whereCondition.payment_transaction.transactionDate.gte = startDate
      }
      if (endDate) {
        whereCondition.payment_transaction.transactionDate.lte = endDate
      }
    }
  ...
  async getMemberPaymentsWithTransactions(userId: string) {
    return await this.prisma.payment.findMany({
  ...
  async getMemberPaymentDetail(paymentId: string, userId: string) {
